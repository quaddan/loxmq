...
| Framework            | Quarkus 3.36.1                                                                           | Stable current. Follows the non-LTS line in rolling-current policy. The closest LTS branches are 3.20 and 3.27 (older) and 3.43 (planned).                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| TLS                  | Quarkus TLS Registry                                                                     | Named TLS configs (server, miniserver, broker) referenced from `application.yml`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
1. `application.yml` (compiled in, contains the defaults and the shape)
...
`server` (see `application.yml`). For outbound calls the default JDK
...
`application-prod.yml`. Quarkus silently falls back to the
...
1. **`SessionOrchestrator.refreshToken()` public** (was
   package-private, called only by the
   24h `TokenRefreshScheduler`). Becomes public to allow
...
| No visible Token section                                                      | Dedicated Token panel with expiresAt + Rights + VALID/EXPIRED badge  |
